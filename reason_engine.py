...
def load_reason_codes():
    """Load reason codes from database, fall back to JSON file."""
    try:
        from db import db_get_reason_codes
        codes = db_get_reason_codes()
        if codes:
            return codes
    except Exception:
        pass
...
def lookup_reason(reason_code, medical_aid, pdf_description=""):
    """
    Look up a reason code and return
    (description, classification, action, fill_colour).
    """
    ...
    if pdf_description:
        ...
        keyword_map = [
            ...
            (["not covered", "not a benefit", "exclusion"],
...
def _classification_to_colour(classification):
    ...
    if "tariff" in cls or "error" in cls or "duplicate" in cls:
        return "AMBER"
